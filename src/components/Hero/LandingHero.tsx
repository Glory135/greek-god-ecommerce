...
const LandingHero = () => {
	...
	if (isPending) {
		return <HeroSkeleton className='min-h-[650px]' />;
	}
	...
	return (
		<section
			style={{
				backgroundImage: `linear-gradient(to bottom, #456d4550, rgba(0, 0, 0, 0.571)), url(${
			...
			<MaxWidthWrapper>
				<div className='relative flex w-fit max-w-full flex-col gap-5'>
					{data?.docs?.title && (
						<h1 className='text-3xl font-bold drop-shadow-lg text-white animate-in slide-in-from-bottom-4 fade-in duration-700 ease-out'>
							{data?.docs?.title}
						</h1>
					)}
