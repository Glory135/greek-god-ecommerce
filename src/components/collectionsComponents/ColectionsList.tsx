...
const CollectionsList = () => {
  const trpc = useTRPC();
  const { data, hasNextPage, isFetchingNextPage, fetchNextPage, isPending, isError } = useSuspenseInfiniteQuery(trpc.collections.getMany.infiniteQueryOptions(
    {
      limit: DEFAULT_LIMIT
    },
    {
  ...
  if (data.pages?.[0]?.docs.length === 0) {
    return (
      <div className="border border-greek border-dashed flex items-center justify-center p-8 flex-col gap-y-5 bg-muted text-primary w-full h-[50vh] rounded-lg">
        <InboxIcon />
        <p className="text-primary text-base font-medium">No Collections Found! </p>
      </div>
    )
  }
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {
          data?.pages.flatMap((page) => page.docs).map((collection) => {
            return (
              <CollectionCard
                key={collection.id}
